...
void NMI_Handler                       (void) __attribute__ ((alias ("Default_Handler"))); //Attribute to call the default handler func so as to we can avoid the undefined reference issue during the linking stage
...
void Reset_Handler(void){
    ...
    for(uint32_t Mem_Counter = 0; Mem_Counter < Section_Size; Mem_Counter++){
